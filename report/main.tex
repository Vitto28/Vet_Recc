...
Table~\ref{tab:ratings_stats} summarises the key figures. As shown in Figure~\ref{fig:rating_dist}, the distribution is heavily skewed: roughly 60\,\% of all reviews are 5-star, producing a characteristic J-shape typical of voluntary online reviews.
 
Both reviews-per-user and reviews-per-item follow power-law distributions, as shown in Figure~\ref{fig:review_activity_dist}. In this dataset, there are 740,985 unique users and 103,288 items, but activity is highly concentrated: the median reviews per user is 1 (max 199), and the median reviews per item is 2 (max 4,109). Notably, 538,197 users (72.6\,\%) have contributed only a single review. The resulting interaction matrix has a sparsity of 99.9984\,\%, which presents a significant challenge for collaborative filtering models.
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.85\linewidth]{figures/review_activity_dist.png}
  \caption{Distribution of reviews per user (left) and per item (right). Both follow power-law distributions with a high concentration of low-activity participants.}
  \label{fig:review_activity_dist}
\end{figure}
...
Sales rank is similarly right-skewed and required log-transformation to produce a more interpretable metric for modeling. As shown in Figure~\ref{fig:sales_rank_dist}, there is a strong negative correlation between log-sales-rank and review volume ($r \approx -0.58$), confirming that lower rank numbers effectively track product popularity. In contrast, the relationship with average rating is much weaker ($r \approx -0.063$), reflecting that popularity and user satisfaction are distinct dimensions. The catalogue is primarily composed of products for \textit{Dogs}, \textit{Cats}, and \textit{Fish \& Aquatics}, which together form the core of the leaf-level category assignments.
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.85\linewidth]{figures/sales_rank.png}
  \includegraphics[width=0.85\linewidth]{figures/sales_rank_vs.png}
  \caption{Sales rank distribution and correlations. The raw distribution is highly skewed (top); lower sales ranks correlate with higher review volume, but show little relationship with average rating (bottom).}
  \label{fig:sales_rank_dist}
\end{figure}
